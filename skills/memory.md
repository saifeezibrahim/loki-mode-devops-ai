...
LOKI_MANAGED_AGENTS=false              <- parent (required for any managed path)
  ...
  |
  +-- REDACTED=false   <- umbrella for multiagent (research preview)
        |
        +-- REDACTED=false    <- code-review council via callable_agents
        +-- REDACTED=false   <- completion council via callable_agents
```
...
When `REDACTED=true`, `run_code_review()` routes
...
Same pattern for `REDACTED` on completion
