...
|----------|-----------|---------|
| `REDACTED()` | 84–105 | Retrieve prior completion verdicts from managed memory store for context augmentation |
...
| `REDACTED()` | 246–264 | Detect stagnation (no changes >= limit) or repeated done signals (>= 2) |
...
| `.loki/managed/council-augment.txt` | Text | `REDACTED()` | 91 | Retrieved prior verdict context from managed memory (if LOKI_MANAGED_MEMORY=true) |
