...
# ---- Case 1: unreachable API REDACTED
reset_events
set +e
OUT=$(
...
if [ "${fb:-0}" -ge 1 ] 2>/dev/null; then
    ...
    echo "SKIP [REDACTED] no fallback event; may be SDK-specific short-circuit"
fi

# ---- Case 2: missing ANTHROPIC_API_KEY REDACTED
reset_events
set +e
OUT=$(
    env -i PATH="$PATH" \
...
# ---- Case 3: SDK not installed (simulated) REDACTED
