...
# --- REASON augment simulation REDACTED
...
# --- REFLECT shadow-write simulation REDACTED
...
if echo "$HARNESS_OUT" | grep -q "^SHADOW_WRITE_OK=True"; then
    ...
    if echo "$HARNESS_OUT" | grep -qE "^FAKE_CALLS=[1-9]"; then
        ...
        bad "REDACTED" "harness_out=$HARNESS_OUT"
    fi
fi
...
if [ "$before_fingerprint" = "$after_fingerprint" ]; then
    ...
    bad "REDACTED" "structure changed before=$before_fingerprint after=$after_fingerprint"
fi
