...
# --- baseline: `loki --help` works REDACTED
set +e
...
# --- `loki start` command exists REDACTED
...
# --- `loki run` command exists REDACTED
...
# --- unified-start detection REDACTED
...
if [ "$ver_rc" -eq 0 ] && echo "$ver_out" | grep -qE "Loki Mode v[0-9]+"; then
    ...
    bad "REDACTED" "rc=$ver_rc out=$ver_out"
fi
