...
# v7.0.0 Phase 4: flag matrix for REDACTED.
#
# Invariants:
...
# Harness: replicates the REDACTED block in run.sh.
...
if [ "$REDACTED" = "true" ] && \
   [ "$REDACTED" != "true" ]; then
    echo "WARNING: REDACTED=true requires REDACTED=true; disabling managed council for this run" >&2
    REDACTED="false"
fi
if [ "$REDACTED" = "true" ] && \
   [ "$LOKI_MANAGED_AGENTS" != "true" ]; then
    echo "WARNING: REDACTED=true requires LOKI_MANAGED_AGENTS=true; disabling managed council for this run" >&2
    REDACTED="false"
fi
if [ "$REDACTED" = "true" ] && \
   [ "$LOKI_MANAGED_AGENTS" != "true" ]; then
    echo "WARNING: REDACTED=true has no effect without LOKI_MANAGED_AGENTS=true" >&2
fi

echo "council=$REDACTED"
'
...
if grep -q "REDACTED=true requires REDACTED=true" autonomy/run.sh; then
...
if grep -q "REDACTED=true requires LOKI_MANAGED_AGENTS=true" autonomy/run.sh; then
...
bootout=$(REDACTED=true ./autonomy/loki version 2>&1 || true)
